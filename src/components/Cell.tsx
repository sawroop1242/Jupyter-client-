...
const renderOutput = (output: ICellOutput, index: number) => {
  switch (output.output_type) {
    case 'stream':
      return (
        <pre key={index} className="text-slate-300 whitespace-pre-wrap font-mono text-sm">
...
const CellToolbar: React.FC<{onRun: ()=>void, onDelete: ()=>void, onAddCell: (type:CellType)=>void, isRunning:boolean, cellType: CellType}> = 
  ({onRun, onDelete, onAddCell, isRunning, cellType}) => (
    <div className="absolute top-2 right-2 z-20 flex items-center space-x-1 p-1 bg-slate-700/50 backdrop-blur-sm rounded-md opacity-0 group-hover:opacity-100 transition-opacity duration-200 border border-slate-600/50">
        ...
        {cellType === 'code' && (
            <button onClick={onRun} disabled={isRunning} className="p-1.5 text-slate-400 hover:bg-slate-600 hover:text-green-400 rounded disabled:text-slate-500" title="Run cell">
      ...
      <div className={commonContainerClasses}>
        ...
        <div className="flex-1 min-w-0">
          ...
          {cell.outputs && cell.outputs.length > 0 && (
            <div className="mt-4 pr-3 pl-1 py-2 bg-slate-900/50 rounded-md">
